            ...
            <div class="products-grid">
                @foreach($products as $product)
                    @php
                        ...
                        // ১. images ফিল্ডে JSON/Array থাকলে প্রথম ছবি নেওয়া
                        if (!empty($product->images)) {
                            if (is_array($product->images)) {
                                $displayImg = $product->images[0] ?? null;
                            } elseif (is_string($product->images)) {
                                $decoded = json_decode($product->images, true);
                                $displayImg = is_array($decoded) ? ($decoded[0] ?? null) : $product->images;
                            }
                        }

                        // ২. images খালি থাকলে সিঙ্গেল image ফিল্ড দেখা
                        if (!$displayImg && !empty($product->image)) {
